import { cn } from "@/lib/utils";
import { IconSettings } from "@tabler/icons-react";

export const Third = () => {
    return(
        <section className="relative flex flex-col items-start  h-full min-h-[400px] md:min-h-[500px pt-5 pl-3 z-10  ">
           
            <FingerprintLogo className="absolute top-33 md:top-49 left-35 md:left-45 w-49 h-35 z-10 text-neutral-400 dark:text-neutral-700"
            style={{
                transform: 'rotateX(30deg) rotateY(-20deg) rotateZ(-8deg) '
            }}
            />

            <Fingerprint3D className="absolute top-62 md:top-76 left-30 md:left-40 w-49 h-35 opacity-70 -z-10 dark:text-neutral-800/60 text-neutral-300/80"
            style={{
                transform: 'rotateX(20deg) rotateY(-20deg) rotateZ(-8deg) '
            }}
            />
            <div className="flex flex-col items-start gap-2 justify-center">
                <h2 className="font-bold text-lg">Approval Queue</h2>
                <p className="text-[16px] text-neutral-600 w-80 md:w-full">Sends agent-generated content to human reviewers before it is published.

            </p>

            </div>
            
            
        <div className="w-full h-full flex justify-center items-center relative perspective-distant md:scale-[1.2]"
              style={{
            transform: 'rotateX(50deg) rotateY(-20deg) rotateZ(40deg) '
        }} 
        >
            <GridBackground />
            <div className="flex scale-[1.2] gap-17 z-20">
                <div className="bg-orange-100 border border-orange-300 flex items-center justify-center gap-3 px-4 py-1 rounded-full w-38 h-7">
                    <IconSettings className="size-4 text-orange-500" />
                    <p className="text-sm text-orange-500 font-medium font-stretch-90%">Processing</p>
                </div>
                <div className="bg-green-100 border border-green-300 flex items-center justify-center gap-3 px-4 py-2 rounded-full w-38 h-7">
                    <GlowingStar className="size-4 text-green-500"  />
                    <p className="text-sm  text-green-500 font-medium font-stretch-90%">Feedback</p>
                </div>
            </div>
        </div>
        </section>
    )
    
};

export const GlowingStar = ({ className }: { className?: string}) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor" // This makes the SVG inherit the text color
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      className={cn("tabler-icon tabler-icon-sparkles", className)}
    >
      <path d="M16 18a2 2 0 0 1 2 2a2 2 0 0 1 2 -2a2 2 0 0 1 -2 -2a2 2 0 0 1 -2 2zm0 -12a2 2 0 0 1 2 2a2 2 0 0 1 2 -2a2 2 0 0 1 -2 -2a2 2 0 0 1 -2 2zm-7 12a6 6 0 0 1 6 -6a6 6 0 0 1 -6 -6a6 6 0 0 1 -6 6a6 6 0 0 1 6 6z" />
    </svg>
  );
};

export const GridBackground= () =>{
    return (
         <div
        className={cn(
          "absolute inset-0 md:left-5 md:right-0 md:inset-y-15 mask-b-from-0% mask-t-from-10% mask-l-from-90% mask-r-from-70% scale[1.2]",
          "bg-size-[40px_40px]",
          "bg-[linear-gradient(to_right,var(--color-neutral-800)_1px,transparent_1px),linear-gradient(to_bottom,var(--color-neutral-800)_1px,transparent_1px)]",
          "dark:bg-[linear-gradient(to_right,var(--color-neutral-300)_1px,transparent_1px),linear-gradient(to_bottom,var(--color-neutral-300)_1px,transparent_1px)]"
        )}
      />

    )
}
export const FingerprintLogo=({className, style}: {className?: string, style?: React.CSSProperties})=>{
  return (
    <svg 
      width="190" 
      height="197" 
      viewBox="0 0 190 197" 
      fill="none" 
      xmlns="http://www.w3.org/2000/svg"
      className={cn("mx-auto", className)}
      style={style}
    >
      <defs>
        {/* All gradient definitions */}
        <linearGradient id="paint0" x1="164.402" y1="136.975" x2="164.402" y2="150.681" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint1" x1="164.402" y1="136.975" x2="164.402" y2="150.681" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint2" x1="147.165" y1="125.474" x2="147.165" y2="185.069" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint3" x1="147.165" y1="125.474" x2="147.165" y2="185.069" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint4" x1="138.747" y1="124.87" x2="138.747" y2="144.334" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint5" x1="138.747" y1="124.87" x2="138.747" y2="144.334" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint6" x1="92.7955" y1="44.5352" x2="92.7955" y2="144.335" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint7" x1="92.7955" y1="44.5352" x2="92.7955" y2="144.335" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint8" x1="17.8023" y1="28.5889" x2="17.8023" y2="76.2796" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint9" x1="17.8023" y1="28.5889" x2="17.8023" y2="76.2796" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint10" x1="32.4584" y1="48.5981" x2="32.4584" y2="75.5674" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint11" x1="32.4584" y1="48.5981" x2="32.4584" y2="75.5674" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint12" x1="94.3364" y1="22.1548" x2="94.3364" y2="150.682" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint13" x1="94.3364" y1="22.1548" x2="94.3364" y2="150.682" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint14" x1="172.814" y1="136.371" x2="172.814" y2="184.469" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint15" x1="172.814" y1="136.371" x2="172.814" y2="184.469" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint16" x1="86.255" y1="49.4242" x2="43.8681" y2="122.841" gradientUnits="userSpaceOnUse">
          <stop stopColor="#CCCCCC" />
          <stop offset="1" stopColor="#F7F7F7" />
        </linearGradient>
        
        <linearGradient id="paint17" x1="86.255" y1="49.4242" x2="43.8681" y2="122.841" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
      </defs>
      
      {/* 3D Layered fingerprint paths - back to front for depth */}
      
      {/* Topmost small detail */}
      <path 
        d="M173.838 137.814L171.849 140.157L154.299 150.29L155.909 148.392C156.218 148.028 156.564 147.734 156.941 147.507L156.94 147.506L173.956 137.684C173.917 137.727 173.876 137.769 173.838 137.814Z" 
        fill="url(#paint0)" 
        stroke="url(#paint1)" 
        strokeWidth="0.209"
      />
      
      {/* Right side layer */}
      <path 
        d="M139.887 126.312C138.543 127.896 138.166 130.42 138.617 133.201C139.069 135.986 140.354 139.05 142.363 141.735L162.848 169.111C164.616 171.474 166.655 173.157 168.616 174.034C170.037 174.669 171.422 174.881 172.634 174.617L155.736 184.373L155.735 184.374C154.354 185.204 152.536 185.146 150.613 184.286C148.692 183.427 146.679 181.77 144.927 179.429L124.442 152.054C122.452 149.393 121.181 146.36 120.735 143.61C120.289 140.857 120.672 138.407 121.958 136.891C122.266 136.527 122.613 136.233 122.99 136.006L122.989 136.005L140.005 126.183C139.966 126.225 139.925 126.268 139.887 126.312Z" 
        fill="url(#paint2)" 
        stroke="url(#paint3)" 
        strokeWidth="0.209"
      />
      
      {/* Right top segment */}
      <path 
        d="M141.083 125.565C142.464 124.735 144.282 124.793 146.205 125.653C148.127 126.512 150.14 128.168 151.892 130.509L154.398 133.859L136.491 144.197L133.971 140.828C132.203 138.465 130.163 136.782 128.202 135.905C126.781 135.269 125.395 135.057 124.184 135.321L141.082 125.566L141.083 125.565Z" 
        fill="currentColor" 
        stroke="url(#paint5)" 
        strokeWidth="0.209"
      />
      
      {/* Middle arc layer */}
      <path 
        d="M49.1787 48.6846C57.7559 43.9064 68.7829 43.1812 81.4541 47.4727C96.8958 52.7025 112.198 64.5162 125.115 79.9258C138.014 95.3135 148.528 114.281 154.427 133.842L136.524 144.178C130.615 124.605 120.093 105.63 107.188 90.2344C94.2544 74.8059 78.9218 62.9636 63.4326 57.7178C52.0516 53.8633 41.9725 54.0401 33.7861 57.5723L49.1797 48.6855L49.1787 48.6846Z" 
        fill="currentColor" 
        stroke="url(#paint7)" 
        strokeWidth="0.209"
      />
      
      {/* Left bottom corner detail */}
      <path 
        d="M32.6836 29.9541C27.3257 33.9229 22.8915 39.3035 19.5361 45.9082C18.531 47.8867 18.5248 50.6511 19.3086 53.5117C20.0933 56.3754 21.6734 59.3523 23.8643 61.7646C25.6167 63.6942 27.4993 64.976 29.2607 65.5664C30.3908 65.9451 31.4741 66.0375 32.4404 65.833L15.7412 75.4746L15.7354 75.4785C14.5361 76.281 12.9587 76.3878 11.2393 75.8115C9.51957 75.2352 7.66557 73.9777 5.93066 72.0674C3.76244 69.6799 2.19809 66.7329 1.42188 63.9004C0.644919 61.0649 0.661842 58.3597 1.63379 56.4463C5.40332 49.0263 10.5358 43.1624 16.8076 39.1191L32.6836 29.9541Z" 
        fill="currentColor" 
        stroke="url(#paint9)" 
        strokeWidth="0.209"
      />
      
      {/* Left middle segment */}
      <path 
        d="M45.6504 50.7241C41.4464 53.8557 37.9631 58.0924 35.3213 63.2925C34.9344 64.0541 34.426 64.6321 33.8281 65.0337L16.6602 74.9448C16.9467 74.6259 17.2028 74.2559 17.4189 73.8306C20.7313 67.3105 25.3688 62.3186 31.0879 59.1323L31.0889 59.1313L45.6504 50.7241Z" 
        fill="currentColor" 
        stroke="url(#paint11)" 
        strokeWidth="0.209"
      />
      
      {/* Large outer arc - back layer for depth */}
      <path 
        d="M34.8965 28.6772C46.0445 21.4906 60.8076 20.0429 77.9434 25.8462C98.1519 32.6904 118.169 48.4327 134.862 68.8999C151.537 89.3453 164.89 114.498 171.799 140.187L153.897 150.523C146.976 124.822 133.617 99.6638 116.937 79.2114C100.228 58.7249 80.1802 42.9521 59.9229 36.0913C45.4734 31.1977 32.685 31.4409 22.3135 35.9419L34.8926 28.6802L34.8965 28.6772Z" 
        fill="currentColor" 
        stroke="url(#paint13)" 
        strokeWidth="0.209"
      />
      
      {/* Right side outer detail */}
      <path 
        d="M175.034 137.066C176.415 136.237 178.234 136.295 180.156 137.154C182.078 138.014 184.091 139.67 185.843 142.011C187.833 144.671 189.104 147.704 189.55 150.454C189.996 153.208 189.614 155.657 188.327 157.174L174.86 173.052C174.552 173.415 174.207 173.708 173.83 173.935L156.812 183.759C156.852 183.716 156.894 183.675 156.932 183.63L170.398 167.752C171.742 166.168 172.119 163.645 171.668 160.863C171.216 158.078 169.931 155.015 167.922 152.329C166.154 149.966 164.114 148.283 162.153 147.406C160.733 146.771 159.348 146.559 158.137 146.822L175.033 137.067L175.034 137.066Z" 
        fill="currentColor" 
        stroke="url(#paint15)" 
        strokeWidth="0.209"
      />
      
      {/* Main front fingerprint path - creates the 3D effect */}
      <path 
        d="M59.8719 36.0767C100.321 49.7761 139.97 99.0269 153.855 150.463L155.759 148.223C158.416 145.091 163.846 146.93 167.887 152.331C171.929 157.732 173.051 164.65 170.394 167.783L156.927 183.661C154.27 186.794 148.84 184.955 144.798 179.553L124.314 152.177C120.272 146.775 119.15 139.857 121.807 136.725C124.464 133.592 129.894 135.431 133.936 140.833L136.261 143.94C124.399 104.933 94.2027 68.3565 63.4171 57.9302C42.2921 50.7758 25.7004 57.5531 17.4104 73.8711C15.4125 77.8038 10.222 77.061 5.81716 72.2109C1.41258 67.3609 -0.538529 60.2408 1.45912 56.3081C11.9621 35.6337 33.0321 26.9868 59.8719 36.0767Z" 
        fill="currentColor" 
        stroke="url(#paint17)" 
        strokeWidth="0.209"
      />
    </svg>
  );
}

export const Fingerprint3D =({ className, style }: { className?: string, style?: React.CSSProperties}) => {
  return (
    <svg 
      width="191" 
      height="198" 
      viewBox="0 0 191 198" 
      fill="none" 
      xmlns="http://www.w3.org/2000/svg"
      className={cn("mx-auto", className)}
      style={style}
    >
      <defs>
        {/* All gradient definitions preserved exactly */}
        <linearGradient id="paint0_linear" x1="170.857" y1="114.011" x2="170.857" y2="138.995" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint1_linear" x1="170.857" y1="114.011" x2="170.857" y2="138.995" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint2_linear" x1="137.822" y1="79.0034" x2="137.822" y2="111.285" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint3_linear" x1="137.822" y1="79.0034" x2="137.822" y2="111.285" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint4_linear" x1="138.887" y1="100.297" x2="138.887" y2="126.567" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint5_linear" x1="138.887" y1="100.297" x2="138.887" y2="126.567" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint6_linear" x1="76.4712" y1="22.1177" x2="76.4712" y2="153.284" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint7_linear" x1="76.4712" y1="22.1177" x2="76.4712" y2="153.284" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint8_linear" x1="72.4292" y1="21.6729" x2="72.4292" y2="130.904" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint9_linear" x1="72.4292" y1="21.6729" x2="72.4292" y2="130.904" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint10_linear" x1="153.002" y1="114.011" x2="153.002" y2="148.57" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint11_linear" x1="153.002" y1="114.011" x2="153.002" y2="148.57" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint12_linear" x1="156.761" y1="74.7373" x2="156.761" y2="138.448" gradientUnits="userSpaceOnUse">
          <stop stopColor="#E6E6E6" />
          <stop offset="1" stopColor="#FAFAFA" />
        </linearGradient>
        
        <linearGradient id="paint13_linear" x1="156.761" y1="74.7373" x2="156.761" y2="138.448" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
        
        <linearGradient id="paint14_linear" x1="86.5734" y1="49.789" x2="44.1865" y2="123.205" gradientUnits="userSpaceOnUse">
          <stop stopColor="#CCCCCC" />
          <stop offset="1" stopColor="#F7F7F7" />
        </linearGradient>
        
        <linearGradient id="paint15_linear" x1="86.5734" y1="49.789" x2="44.1865" y2="123.205" gradientUnits="userSpaceOnUse">
          <stop />
          <stop offset="1" stopOpacity="0.5" />
        </linearGradient>
      </defs>
      
      {/* All 8 layers of the 3D fingerprint - preserved in exact order for depth */}
      
      {/* Layer 1: Top right segment */}
      <path 
        d="M174.147 118.72C175.468 121.803 177.479 124.447 179.665 126.231C181.848 128.012 184.222 128.948 186.265 128.577C186.322 128.566 186.379 128.55 186.436 128.538L169.421 138.362C169.036 138.575 168.609 138.729 168.14 138.814C166.183 139.17 163.87 138.276 161.709 136.512C159.551 134.751 157.559 132.134 156.251 129.081L154.286 124.498L172.192 114.161L174.147 118.72Z" 
        fill="currentColor" 
        stroke="url(#paint1_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 2: Middle left curved segment */}
      <path 
        d="M140.314 79.7827C139.481 80.6997 138.973 82.0056 138.813 83.5522C138.593 85.6888 139.03 88.2967 140.192 91.0093C141.513 94.092 143.525 96.7371 145.711 98.521C147.894 100.302 150.268 101.237 152.311 100.866L154.296 100.505L136.743 110.639L134.186 111.104C132.229 111.46 129.916 110.566 127.755 108.802C125.597 107.041 123.605 104.424 122.297 101.371C121.145 98.6828 120.716 96.1109 120.933 94.0171C121.149 91.9223 122.009 90.319 123.418 89.5376L123.42 89.5366L140.314 79.7827Z" 
        fill="currentColor" 
        stroke="url(#paint3_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 3: Small connecting segment */}
      <path 
        d="M154.632 100.554C151.343 107.547 146.655 112.729 140.941 116.033L125.927 124.701C130.414 121.364 134.133 116.721 136.862 110.812L154.632 100.554Z" 
        fill="currentColor" 
        stroke="url(#paint5_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 4: Large bottom left arc */}
      <path 
        d="M20.6113 23.0103C19.9515 23.7448 19.4908 24.7294 19.2539 25.897C18.8845 27.7178 19.0537 29.9896 19.8486 32.4722C30.3457 65.2557 51.403 98.213 78.2305 120.105C104.591 141.616 130.64 147.647 149.445 139.397L133.717 148.477C114.872 158.355 87.7751 152.828 60.2744 130.386C33.4816 108.523 12.4461 75.6006 1.95996 42.8511C1.17316 40.3937 1.0106 38.1598 1.37109 36.3823C1.73159 34.6052 2.61204 33.2923 3.90625 32.6548H3.90723L3.91309 32.6509L20.6113 23.0103Z" 
        fill="currentColor" 
        stroke="url(#paint7_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 5: Outer left arc layer */}
      <path 
        d="M24.2568 21.7852C26.4 21.9 28.7511 23.2382 30.8184 25.3291C32.8834 27.4179 34.6532 30.2467 35.6367 33.3184C43.9436 59.2618 60.5692 85.2513 81.7412 102.528C102.487 119.457 122.995 124.249 137.84 117.826L122.855 126.478C107.955 135.094 86.0412 130.971 63.7852 112.81C42.6479 95.5609 26.044 69.6066 17.748 43.6973C16.7543 40.5938 14.9665 37.7366 12.8789 35.625C10.7934 33.5156 8.39574 32.1383 6.17969 32.0195C5.7036 31.9941 5.25514 32.0296 4.83594 32.1182L22.002 22.207C22.6487 21.8899 23.4037 21.7395 24.2568 21.7852Z" 
        fill="currentColor" 
        stroke="url(#paint9_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 6: Right middle connecting piece */}
      <path 
        d="M172.027 114.252C167.608 125.449 160.603 133.423 151.808 138.035L151.804 138.037L138.812 145.537C145.385 140.705 150.667 133.675 154.232 124.526L172.027 114.252Z" 
        fill="currentColor" 
        stroke="url(#paint11_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 7: Large right outer arc */}
      <path 
        d="M163.273 74.9189C165.23 74.5634 167.544 75.457 169.705 77.2207C171.863 78.9819 173.854 81.5984 175.163 84.6523L188.63 116.081C189.782 118.769 190.21 121.34 189.993 123.434C189.777 125.529 188.917 127.132 187.508 127.913L187.507 127.914L170.61 137.668C171.444 136.751 171.953 135.446 172.113 133.898C172.334 131.762 171.896 129.154 170.733 126.441L157.267 95.0137C155.945 91.9308 153.935 89.2858 151.749 87.502C149.566 85.7207 147.192 84.7851 145.148 85.1562L124.664 88.8799C124.605 88.8906 124.548 88.9053 124.49 88.918L141.508 79.0938C141.893 78.8803 142.321 78.7269 142.79 78.6416L163.273 74.9189Z" 
        fill="currentColor" 
        stroke="url(#paint13_linear)" 
        strokeWidth="0.208864"
      />
      
      {/* Layer 8: Main front fingerprint path - creates primary 3D effect */}
      <path 
        d="M145.117 85.1484C149.158 84.4138 154.588 88.8446 157.245 95.0454L170.712 126.474C173.369 132.675 172.247 138.297 168.206 139.032C164.164 139.766 158.734 135.335 156.077 129.135L154.173 124.695C140.288 160.098 100.64 163.569 60.1902 130.561C33.3505 108.659 12.2805 75.6821 1.77748 42.8799C-0.220302 36.6404 1.73092 31.7733 6.13552 32.0093C10.5402 32.2454 15.7307 37.4954 17.7288 43.7349C26.0187 69.6254 42.6105 95.5611 63.7355 112.8C94.5212 137.922 124.717 136.213 136.58 110.903L134.254 111.326C130.213 112.061 124.782 107.63 122.125 101.429C119.468 95.2284 120.591 89.6053 124.632 88.8706L145.117 85.1484Z" 
        fill="currentColor" 
        stroke="url(#paint15_linear)" 
        strokeWidth="0.208864"
      />
    </svg>
  );
}


// const [inputNumber, setInputNumber] = useState<number | string>("");

//     // The calculation logic
//     const calculatePercentage = (num: number) => {
//         if (!num) return 0;
//         const total = 1350;
//         return (num * 100) / total;
//     };

//     const result = typeof inputNumber === "number" ? calculatePercentage(inputNumber) : 0;

//     // return (
    //     <div className="flex flex-col justify-center items-center w-full h-full bg-neutral-50 gap-6">
    //         <div className="p-8 bg-white border border-neutral-200 rounded-xl shadow-sm flex flex-col gap-4 w-80">
    //             <h2 className="text-lg font-semibold text-neutral-800">Percentage Calculator</h2>
                
    //             <div className="flex flex-col gap-2">
    //                 <label className="text-sm text-neutral-500">Value (out of 1350)</label>
    //                 <input 
    //                     value={inputNumber}
    //                     onChange={(e) => setInputNumber(e.target.value === "" ? "" : Number(e.target.value))}
    //                     type="number" 
    //                     placeholder="Enter your number" 
    //                     className="w-full h-10 px-3 border border-neutral-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
    //                 />
    //             </div>

    //             <div className="mt-4 p-4 bg-neutral-100 rounded-lg text-center">
    //                 <p className="text-sm text-neutral-500 uppercase tracking-wider font-bold">Result</p>
    //                 <p className="text-3xl font-mono font-bold text-blue-600">
    //                     {result.toFixed(1)}%
    //                 </p>
    //             </div>
    //         </div>
            
    //         <p className="text-xs text-neutral-400">
    //             Formula used: $$(Input \times 100) / 1350$$
    //         </p>
    //     </div>
    // );