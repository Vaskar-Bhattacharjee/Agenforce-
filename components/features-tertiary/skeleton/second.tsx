...
export const Second = ({ className }: { className?: string }) => {
  return (
    <div className={cn("p-4", className)}>
      ...
      <p className="text-neutral-600 w-120 mt-2 mb-8 md:mb-0">
        Coordinate multiple agents across workflows using memory, interrupts,
        and conditional logic.
      </p>
      <CardSkeleton className="h-[calc(100%-100px)] md:h-full w-fit lg:min-w-full flex flex-col gap-y-2.5 justify-center items-center mask-r-from-70% mask-b-from-70% mask-t-from-70% mask-l-from-70% md:-mt-10 relative">
         <Admin />
...
export const Item = ({
  ...
  return (
    <div
      className={cn(
        "size-17 md:size-20  border border-dashed border-neutral-300 rounded-xl overflow-hidden relative ",
    ...
    >
       
      {
        src && (
          <div
            className={cn(
              "h-full p-[2.8px] w-full bg-linear-to-br from-blue-400 via-transparent  to-blue-400 flex items-center justify-center overflow-hidden whitespace-nowrap z-10",
              className
            )}
          >
            <Image
        ...
        {!src && children && (
              <div
            className={cn(
              "h-full p-[2.8px] w-full bg-linear-to-br from-neutral-400 via-transparent  to-neutral-400 flex items-center justify-center overflow-hidden whitespace-nowrap z-10",
              className
            )}
          >
            ...
            <div
              className={cn(
                "object-cover  z-12 w-full h-full rounded-[12px] overflow-hidden",
                childrenBg
              )}
            >
              {children}
...
export const Admin = () => {
    return (
        <div className="absolute flex flex-col items-center gap-1 top-14 md:top-25 -left-2 md:left-20 z-20">
            <div className="flex items-center justify-end w-28 h-4">
                <IconPointerFilled className="rotate-y-180 -rotate-z-20 text-neutral-500 dark:text-neutral-700" />
            </div>
            <p className="text-neutral-50 dark:text-neutral-50 bg-gray-500 border border-neutral-400 dark:border-neutral-700 px-6 py-0.5 rounded-xl z-50">Admin</p>
        </div>
    )
}